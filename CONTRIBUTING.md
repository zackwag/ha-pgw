# Contributing to ha-pgw

Thanks for your interest in improving this Home Assistant integration for Philadelphia Gas Works (PGW).

## Getting started

```sh
git clone https://github.com/zackwag/ha-pgw.git
cd ha-pgw
pip install .[test]
```
...
The integration lives in `custom_components/pgw/` and depends on the `pgw-api` PyPI package. Tests live in `tests/` and use `pytest`:
